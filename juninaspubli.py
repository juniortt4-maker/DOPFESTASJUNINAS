...
try:
    ...
    coluna_tipo_publico = localizar_coluna(colunas, ["TIPO PUBLICO"])
    ...
    df = tratar_coluna_categorica(df, coluna_comando)
    df = tratar_coluna_categorica(df, coluna_cidade)
    df = tratar_coluna_categorica(df, coluna_evento)
    ...
    if "Ano" in df_filtrado.columns and df_filtrado["Ano"].notna().any():
        ...
        anos = sorted(df_filtrado["Ano"].dropna().astype(int).unique())
    ...
    if coluna_comando:
        ...
        if comandos_sel:
            df_filtrado = df_filtrado[
                df_filtrado[coluna_comando].isin(comandos_sel)
            ]
    ...
    if coluna_cidade:
        ...
        if cidades_sel:
            df_filtrado = df_filtrado[
                df_filtrado[coluna_cidade].isin(cidades_sel)
            ]
    ...
    if coluna_natureza:
        ...
        if natureza_sel:
            df_filtrado = df_filtrado[
                df_filtrado[coluna_natureza].isin(natureza_sel)
            ]
    ...
    if coluna_tipo_publico:
        ...
        if tipos_sel:
            df_filtrado = df_filtrado[
                df_filtrado[coluna_tipo_publico].isin(tipos_sel)
            ]
    ...
    total_publico = int(
        df_filtrado[coluna_publico].fillna(0).sum()
    ...
    # COMPARATIVO ANO/MÊS
    # =========================
    if "Ano" in df_filtrado.columns and "Mes_Abrev" in df_filtrado.columns:
